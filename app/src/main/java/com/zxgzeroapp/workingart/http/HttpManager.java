...
import com.zxgzeroapp.workingart.http.interceptor.UpdateLoggingInterceptor;
...
//                .map(basePar);
     ...
     * @param basePar 封装的请求数据
     */
    public static void doHttpDealForUpdate(BaseApi basePar) {
        //手动创建一个OkHttpClient并设置超时时间缓存等设置
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        builder.connectTimeout(basePar.getConnectionTime(), TimeUnit.SECONDS);
        builder.addInterceptor(new UpdateLoggingInterceptor());
        builder.interceptors().add(new ReceivedCookiesInterceptor(basePar.getRxAppCompatActivity()));
        builder.interceptors().add(new AddCookiesInterceptor(basePar.getRxAppCompatActivity()));
...
//                .map(basePar);
     ...
     * @param basePar 封装的请求数据
     */
    public static void doHttpDeal(BaseApi basePar,String URL) {
        //手动创建一个OkHttpClient并设置超时时间缓存等设置
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        builder.connectTimeout(basePar.getConnectionTime(), TimeUnit.SECONDS);
        builder.addInterceptor(new LoggingInterceptor());
